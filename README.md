...
- `Ctrl + Space`: 入力モード切替（英数/かな）
- `` Alt + ` ``: 入力モード切替（英数/かな）
- `VK_IME_ON` (`0x16`): かな入力へ切替
- `VK_IME_OFF` (`0x1A`): 英数入力へ切替

`Ctrl + Space` と `` Alt + ` `` は設定画面から個別に ON/OFF できます（デフォルトは ON）。

`Shift + Space` は一般的なアプリのショートカットと競合しやすいため、現在は未対応です。

## 数字・記号の入力幅

日本語入力中の数字・記号は、設定画面の「全般」から記号ごとに全角/半角を設定できます。

- 対象: `0-9` と主要な記号（`!`, `,`, `.`, `/` など）
- プリセット: `すべて半角` / `すべて全角` / `既定に戻す`
...
全角時の記号変換は、Unicodeの一般的な全角記号ではなく、azooKeyの既存ルールに準拠します。
